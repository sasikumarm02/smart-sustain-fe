...
const WaterPDF = (report: any) => {
  const { user } = useAuth();
  const [isLoading, setIsLoading] = useState(false);
  const [stationary, setStationaty] = useState([]);
  const [process, setProcess] = useState([]);
  const [mobile, setMobile] = useState([]);
  const [fugitive, setFugitive] = useState([]);
  ...
  const fetchPdf = async (pdfData: any, reportYear: any) => {
    ...
    const totalWaterConsumption = pdfData[0]['Total Water Consumption (in m³)']
      ? formatNumberUS(pdfData[0]['Total Water Consumption (in m³)'])
      : '0';
    const waterConsumptionDetails = pdfData[0]['Water Consumption'];

    const generateWaterPieChart = (waterConsumptionDetails: any) => {
      const aggregatedData = waterConsumptionDetails.reduce(
        (acc: any, record: any) => {
          if (
            record['Source of Water'] &&
            record['Actual Water Consumption (in m³)'] !== undefined
          ) {
            acc[record['Source of Water']] =
              record['Actual Water Consumption (in m³)'];
          }
          return acc;
        },
        {}
      ...
      // Generate legends dynamically with colors
      const legendItems = Object.keys(aggregatedData).map((source, index) => {
        const colors = [
          '#4D9DE0',
          '#E15554',
          '#3BB273',
        ...
        return {
          columns: [
            ...
            },
            {
              width: 'auto',
              text: source,
              margin: [5, 0, 0, 0], // Adjust spacing
              fontSize: 10,
              bold: true,
            },
          ],
          margin: [0, 2, 0, 2], // Adjust vertical spacing
        };
      });

      // Return both chart and legends
      return {
        columns: [
          // {
          //   stack: legendItems, // Add the legends here
          //   margin: [30, 0, 0, 0],
          // },
          {
            image: pieChartImage,
    ...
    const pieChartImage = generateWaterPieChart(
      pdfData[0]['Water Consumption']
    );
    ...
    const docDefinition: any = {
      content: [
        ...
        {
          text: `Total Water Consumption: ${totalWaterConsumption}m³`,
          style: 'subtitle',
        },
        // {
        //   ...pieChartImage, // First chart (Gender)
        // },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            widths: [50, 228, 228, 228],
            body: [
              [
                { text: 'S.No.', style: 'tableHeader' },
                { text: 'Source of Water', style: 'tableHeader' },
                { text: 'Target Consumption (in m³)', style: 'tableHeader' },
                {
                  text: 'Actual Water Consumption (in m³)',
                  style: 'tableHeader',
                },
              ],
              ...(waterConsumptionDetails.length > 0
                ? waterConsumptionDetails.map((item: any, ind: any) => [
                    { text: ind + 1, style: 'tableContent' },
                    { text: item['Source of Water'], style: 'tableContent' },
                    {
                      text: item['Target Consumption (in m³)'],
                      style: 'tableContent',
                    },
                    {
                      text: formatNumberUS(
                        item['Actual Water Consumption (in m³)']
                      ),
                      style: 'tableContent',
                    },
                  ])
    ...
    pdfMake
      ...
      .download('Water Withdrawal Consumption Report.pdf');
  };
  function fetchApiData() {
    setIsLoading(true);
    get(
      `/water/download_water_consumption_data/?entity_Id=${user.entity_Id}${facilitySelected ? `&facility_Id=${facilitySelected}` : ''}`
    )
      .then((res: any) => {
