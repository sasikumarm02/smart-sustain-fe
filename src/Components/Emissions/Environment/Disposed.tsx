...
const DisposedForm = () => {
  ...
  const fetchFeildValues = (index: any) => {
    get(
      `/waste/get_prefetched_waste_data/?entity_Id=${user.entity_Id}&waste_category=${selectedWasteCat}&waste_type=${selectedWasteType}&process=${selectedProcess}&waste_management_type='Disposed'&facility_Id=${facilitySelected}`
    )
      .then((res: any) => {
  ...
  function transformFormikResponse(formikResponse: any) {
    return {
      ...
      waste_data: formikResponse?.waste_data.map((data: any) => ({
        ...
        waste_management_type: 'Disposed',
        process: data?.disposal_method,
        disclosure: data?.disclosure,
        monthly_data: Object.entries(data.monthlyData)
          .filter(([month, details]: any) => Number(details.quantity > 0))
          .map(([month, details]: any) => ({
            waste_in_tonnes: details.quantity,
            month: month,
          })),
      })),
    };
  ...
  function transformFormikExcelResponse(formikResponse: any) {
    return {
      entity_Id: user?.entity_Id,
      facility_Id: facilitySelected,
      waste_data: formikResponse?.waste_disposal_data.map((data: any) => ({
        waste_category: data?.waste_category,
        waste_type: data?.waste_type,
        waste_management_type: 'Disposed',
        process: data?.disposal_method,
  ...
  const DataEntryDispose = () => {
    ...
    interface response {
      entity_Id: string;
      waste_disposal_data: MobileDatum[];
    }
    ...
    function initializeForm(response: response) {
      ...
      const { waste_disposal_data } = response;
      const data = waste_disposal_data?.map((item) => {
        return {
        ...
        <Row gutter={10} className="mt-4">
          {!ExcelUploaded ? (
            <>
              ...
              <Col span={24}>
                <CustomDownload type={'Waste_Management_Disposed'} />
              </Col>
            </>
          ) : (
            <></>
          ...
          <Col span={24}>
            {ExcelUploaded ? (
              <Formik
                initialValues={{
                  entity_Id: '',
                  waste_disposal_data: initalFormValues,
                }}
                validationSchema={Yup.object().shape({
                  waste_disposal_data: Yup.array().of(
                    Yup.object()
                      .shape({
                        ...
                        disposal_method: Yup.string().required(
                          'disposal_method is required'
                        ),
                  ...
                  >
                    <div className={Styles.formFooter}>
                      <FieldArray name="waste">
                        {(arrayHelpers) => (
                          <>
                            {values?.waste_disposal_data?.map(
                                  ...
                                  >
                                    <Col xl={4} lg={4} md={12} sm={24}>
                                      <Select
                                        ...
                                        disabled={true}
                                        value={
                                          values?.waste_disposal_data[index]
                                            ?.Waste_Category
                                        }
                                        onChange={(value) =>
                                          setFieldValue(
                                            `waste_disposal_data.${index}.Waste_Category`,
                                            value
                                          )
                                        }
                                        style={{
                                    ...
                                    <Col xl={4} lg={4} md={12} sm={24}>
                                      <Input
                                        ...
                                        onChange={(e) => {
                                          setFieldValue(
                                            `waste_data[${index}].disposal_method`,
                                            e.target.value
                                          );
                                        }}
                                        style={{ height: '40px' }}
                                        ...
                                        <Col>
                                          ...
                                          >
                                            <Input
                                              ...
                                              onChange={(e) =>
                                                setFieldValue(
                                                  `waste_disposal_data.${index}.monthlyData.${
                                                    monthsForCurrentYear[
                                                      idx - 1
                                                    ]
                                                  }.quantity`,
                                    ...
                                    <Col xl={4} lg={4} md={12} sm={24}>
                                      <Form.Item>
                                        <Select
                                          ...
                                          placeholder="Waste Type"
                                          options={
                                            !isEmpty(dropDownWasteType)
                                              ? dropDownWasteType.map(
                                                  (data) => ({
                                                    label: data,
                                                    value: data,
                                                  })
