...
export default function CategoryFiveForm({ setActiveView }: { setActiveView?: (view: string) => void }) {
  const [activeTab, setActiveTab] = useState('Supplier-specific Method');
  const [entryMode, setEntryMode] = useState('Manual');
  const [data, setData] = useState<any[]>([{}]);

  const TABS = [
    { id: 'Supplier-specific Method', label: 'Supplier-specific Method' },
    { id: 'Waste-type-specific Method', label: 'Waste-type-specific Method' },
  ...
  const columns = [
    {
      ...
      render: (text: any, record: any, rowIndex: number) => (
        <SelectComponent options={[{ label: 'Plastic', value: 'Plastic' }, { label: 'Paper', value: 'Paper' }]} value="" placeHolder="Select"
        // customClass="w-full"
        />
      )
    },
    {
      ...
      key: 'uom',
      render: (text: any, record: any, rowIndex: number) => (
        <SelectComponent options={[{ label: 'tonnes', value: 'tonnes' }, { label: 'kg', value: 'kg' }]} value="" placeHolder="Select"
        // customClass="w-full" 
        />
      )
    },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center text-sm mb-4">
        ...
        <span className="mx-2 text-gray-400">&gt;</span>
        <span className="font-bold text-[#001D3D] text-lg cursor-pointer hover:text-[#036323]" onClick={() => setActiveView && setActiveView('scope-three-cat5')}>Category 5</span>
        <span className="mx-2 text-gray-400">&gt;</span>
      ...
      <PageCardComponent customClass="p-8 pb-4 bg-white shadow-sm rounded-xl">
        ...
        <div className="flex justify-end gap-3 pt-12 pb-4">
          <button type="button" className="text-[#036323] font-bold text-sm px-6 py-2" onClick={() => setActiveView && setActiveView('scope-three-cat5')}>Cancel</button>
