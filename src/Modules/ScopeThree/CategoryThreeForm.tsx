...
export default function CategoryThreeForm({ setActiveView }: { setActiveView?: (view: string) => void }) {
  const [activeTab, setActiveTab] = useState('Upstream emissions of purchased fuels');
  const [entryMode, setEntryMode] = useState('Manual');
  const [data, setData] = useState<any[]>([{}]);

  const TABS = [
    { id: 'Upstream emissions of purchased fuels', label: 'Upstream emissions of purchased fuels' },
    { id: 'Upstream emissions of purchased electricity', label: 'Upstream emissions of purchased electricity' },
    { id: 'T&D losses', label: 'T&D losses' },
  ];
  ...
  const columns = [
    {
      ...
      render: (text: any, record: any, rowIndex: number) => (
        <SelectComponent options={[{ label: 'Diesel', value: 'Diesel' }, { label: 'Petrol', value: 'Petrol' }]} value="" placeHolder="Select" />
      )
    },
    {
      title: 'UOM*',
      ...
      key: 'uom',
      render: (text: any, record: any, rowIndex: number) => (
        <SelectComponent options={[{ label: 'kWh', value: 'kWh' }, { label: 'Liters', value: 'Liters' }]} value="" placeHolder="Select" />
      )
    },
    {
      title: 'Quantity*',
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center text-sm mb-4">
        ...
        <span className="mx-2 text-gray-400">&gt;</span>
        <span className="font-bold text-[#001D3D] text-lg cursor-pointer hover:text-[#036323]" onClick={() => setActiveView && setActiveView('scope-three-cat3')}>Category 3</span>
        <span className="mx-2 text-gray-400">&gt;</span>
      ...
      <PageCardComponent customClass="p-8 pb-4 bg-white shadow-sm rounded-xl">
        ...
        <div className="flex justify-end gap-3 pt-12 pb-4">
          <button type="button" className="text-[#036323] font-bold text-sm px-6 py-2" onClick={() => setActiveView && setActiveView('scope-three-cat3')}>Cancel</button>
