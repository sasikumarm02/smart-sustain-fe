...
export default function CategorySixForm({ setActiveView }: { setActiveView?: (view: string) => void }) {
  const [activeTab, setActiveTab] = useState('Fuel-based Method');
  const [entryMode, setEntryMode] = useState('Manual');
  const [data, setData] = useState<any[]>([{}]);

  const TABS = [
    { id: 'Fuel-based Method', label: 'Fuel-based Method' },
    { id: 'Distance-based Method', label: 'Distance-based Method' },
  ...
  const columns = [
    {
      ...
      render: (text: any, record: any, rowIndex: number) => (
        <SelectComponent options={[{ label: 'Air Travel', value: 'Air Travel' }, { label: 'Train', value: 'Train' }]} value="" placeHolder="Select" />
      )
    },
    {
      title: 'UOM*',
      ...
      key: 'uom',
      render: (text: any, record: any, rowIndex: number) => (
        <SelectComponent options={[{ label: 'km', value: 'km' }, { label: 'miles', value: 'miles' }]} value="" placeHolder="Select" />
      )
    },
    {
      title: 'Quantity*',
      ...
      key: 'quantity',
      render: (text: any, record: any, rowIndex: number) => (
        <InputComponent type="number" placeHolder="Enter quantity" />
      )
    },
    {
      title: 'Month*',
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center text-sm mb-4">
        ...
        <span className="mx-2 text-gray-400">&gt;</span>
        <span className="font-bold text-[#001D3D] text-lg cursor-pointer hover:text-[#036323]" onClick={() => setActiveView && setActiveView('scope-three-cat6')}>Category 6</span>
        <span className="mx-2 text-gray-400">&gt;</span>
      ...
      <PageCardComponent customClass="p-8 pb-4 bg-white shadow-sm rounded-xl">
        ...
        <div className="flex justify-end gap-3 pt-12 pb-4">
          <button type="button" className="text-[#036323] font-bold text-sm px-6 py-2" onClick={() => setActiveView && setActiveView('scope-three-cat6')}>Cancel</button>
